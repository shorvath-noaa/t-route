...
from .preset_parameters import PresetParameters
...
class Config(BaseModel, extra='forbid'):
    ...
    preset_parameters: PresetParameters = Field(default_factory=dict)
    ...
    ################################################################################
    # Use preset values for operational configuarions.
    ################################################################################
    @root_validator(pre=True)
    def check_predefined_configurations(cls, values):
        # If custom configuration, don't use any preset parameters
        preset_parameters = values['preset_parameters']
        if preset_parameters:
            preset_config = preset_parameters['preset_configuration']
        else:
            preset_config = 'Custom'
        
        if preset_config != 'Custom':
            #-----------------------------------------------
            # Set values based on predefined configurations:
            #-----------------------------------------------
            #NOTE These are based on information found here:
            # https://www.nco.ncep.noaa.gov/pmb/codes/nwprod/nwm.v2.2.3/parm/
            
            ### Number of time steps
            #NOTE This is assuming t-route is using default 5 minute (300 sec) time step.
            # If user does not want to use this default, they must use a "Custom" configuration.
            #TODO: Generalize this to use preset configurations with unique time steps??
            nts_dict = {"Standard_AnA": 36,
                        "LongRange_AnA": 36, #144?? Website says 3 hour run #TODO: Verify... 
                        "Extended_AnA": 336,
                        "ShortRange_Forecast": 216, 
                        "MediumRange_Forecast": 2880, 
                        "LongRange_Forecast": 8640}
            values['compute_parameters']['forcing_parameters']['nts'] = nts_dict[preset_config]
            
            ### Data Assimilation
            # Streamflow Nudging
            values['compute_parameters']['data_assimilation_parameters']['streamflow_da']['streamflow_nudging'] = True
            
            # Reservoir USGS/USACE Persistence
            if preset_config in ['Standard_AnA', 'ShortRange_Forecast', 'MediumRange_Forecast']:
                reservoir_persistence = True
            else:
                reservoir_persistence = False
            values['compute_parameters']['data_assimilation_parameters']['reservoir_da']['reservoir_persistence_da']['reservoir_persistence_usgs'] = reservoir_persistence
            values['compute_parameters']['data_assimilation_parameters']['reservoir_da']['reservoir_persistence_da']['reservoir_persistence_usace'] = reservoir_persistence
            
            # Reservoir RFC Forecast
            if preset_config in ['LongRange_AnA', 'LongRange_Forecast']:
                rfc_forecast = False
            else:
                rfc_forecast = True
            values['compute_parameters']['data_assimilation_parameters']['reservoir_da']['reservoir_rfc_da']['reservoir_rfc_forecasts'] = rfc_forecast
